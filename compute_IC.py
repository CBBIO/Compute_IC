...
def get_depth(pronto_Term):
    ...
    while entity_iterator:
        ...
        for x in entity_iterator:
            parents = [y for y in x.superclasses(distance=1, with_self=False)]
...
def process_file(file_data):
    data = [x + [obo[x[1]][1], obo[x[1]][0]] for x in file_data]
    return data
...
def crow(row_data):
    ...
    if godag.get(goterm, None):
        ...
        ic_row = [
            ...
            aspect,
            goterm,
            len(sorted(entity.subclasses(with_self=False))),
            get_depth(entity),
...
def compute_bk(input_data, annotations, godag):
    input_data_ic = []
    for prot_id, goterm, aspect, desc in input_data:
        goterm = goterm.strip()
        if godag.get(goterm, None):
            entity = godag[goterm]
            ic_row = [
            ...
            input_data_ic.append(ic_row)
    return input_data_ic


def compute(input_data, annotations, godag):
    input_data_ic = [x for x in p_tqdm.p_umap(
        crow, input_data, num_cpus=CPU_COUNT) if x]
    return input_data_ic
...
if __name__ == "__main__":
    ...
    parser.add_argument("--annotation", help="Gzipped GPAD file \".gpad.gz\"")
    args = parser.parse_args()

    if os.path.exists(args.inputfile):
        input_data=read_input(args.inputfile)
        if args.annotation:
            if os.path.exists(args.annotation):
                annotations = compile_universe(read_gpad(args.annotation))
            else:
                print("Bad annotations file")
        else:
            # No annotations file specified. We'll use a "universe", compiling all annotations in GeneOntology
            annotations = get_universe(True)  # Progress Bar ON
    else:
        print("Wrong inputfile")

    print("Computing ICs")
    ic_data = compute(input_data, annotations, godag)
    print("Writing data file")
    dump_ic_data(
        ic_data, outputfile=args.inputfile.split(
            "/")[-1].split(".")[0] + ".tsv"
    )
    print("Plotting results")
    plot_density(ic_data, args.inputfile.split("/")[-1].split(".")[0])
